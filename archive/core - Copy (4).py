...
while True:
    ...
    for mainer in mainers:	
	    ...
	    preposition_x = mainer.pos[0]
	    preposition_y = mainer.pos[1]
	    cheker = 0
	    for object in mainers:
             avoid = []
             cheker = 0
             cheker_2 = 0
             preposition_x = mainer.pos[0] + cos*object.r_size*4
             preposition_y = mainer.pos[1] + sin*object.r_size*4
             nearposition_x = mainer.pos[0] + cos
             nearposition_y = mainer.pos[1] + sin
             if object == mainer:
                 cheker-=10
                 cheker_2-=10  				 
             ...
             if object.pos[0]+object.r_size>nearposition_x>object.pos[0]-object.r_size: cheker_2+=1
             if object.pos[1]+object.r_size>nearposition_y>object.pos[1]-object.r_size: cheker_2+=1
             if cheker_2 == 2 or mainer.take_1 > 0:
                 if mainer.take_1 == 0 and cheker_2 == 2:
                    mainer.take_1 = 31
                    #avoid.append(object)
                 elif mainer.take_1 > 0: mainer.take_1-=1
                 if mainer.rot[1]<=84: mainer.rot[1]+=3
                 if mainer.rot[1]>1 and mainer.rot[1]<84: mainer.speed = 0
                 if mainer.rot[1]<1: mainer.speed = -1
                 if mainer.rot[1]>=84: mainer.speed = 1
                 #if mainer.take_1 == 0: avoid.remove(object)
             if cheker == 2:
                 if mainer.rot[1]<42: mainer.rot[1]+=3
                 break
             elif mainer.rot[1]>0.1 and cheker<2: mainer.rot[1]-=0.1
	    cos = cos*math.cos(math.radians(mainer.rot[1]))-sin*math.sin(math.radians(mainer.rot[1]))
	    sin = sin*math.cos(math.radians(mainer.rot[1]))+cos*math.sin(math.radians(mainer.rot[1]))
	    mainer.pos[0]+=cos*mainer.speed
	    mainer.pos[1]+=sin*mainer.speed
    ...
    text = font.render("Score: cos 1 "+str(mainer.speed),True,black)
    sc.blit(text, [0,0])
    time.sleep(0.01)
    pygame.display.flip()
