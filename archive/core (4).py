...
class Marine(Mainer):
    def __init__(self,pos=(0,0),rot=(0,0,0),data_protections=(100,10,10,10)):	
        ...
        self.targets.insert(0, Target(0,(self.pos[0]+self.cos*5,self.pos[1]+self.sin*5)))
        self.take = 1	
        self.order_t = -1	
        self.dog = self.just_dog		
    def getChosen(self):
    ...
    def getTake(self):	
        ...
        if self.take == 3:
             if len(self.targets)>self.order_t*(-1):
                         ...
                         self.targets.pop()
                         self.targets.insert(0, Target(0,(self.pos[0]+self.cos*50,self.pos[1]+self.sin*50)))
                         self.order_t = -1
                         self.take = 1
...
while True:
    ...
    text = font.render("Minerals: "+str(resours.res[0])+"Minerals: "+str(mainer.take),True,black)
    sc.blit(text, [0,0]) 
    time.sleep(0.001)
    pygame.display.flip()
