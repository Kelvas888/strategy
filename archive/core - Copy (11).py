...
class Cam:
    ...
    def findobject(self,pos_m=(0,0)):
        ...
        for self.object in mainers:
             ...
             if self.cheker == 2:
                  ...
                  if self.onclick == 1 and self.choosObject[-1].takeclick == 1:
                         targets.insert(0, Target((pos_m[0]-cam.pos[0],pos_m[1]-cam.pos[1])))
                         for select_object in self.choosObject:
        ...
        if self.choosArea == 1:
             if self.areaEvent == 0:
                  self.rectPosition[0] = pos_m[0]
                  self.rectPosition[1] = pos_m[1]
                  self.rectPosition[2] = pos_m[0]
                  self.rectPosition[3] = pos_m[1]
                  self.areaEvent = 1
...
while True:
    ...
    for crystal in crystals:
        crystal.switching()
    pygame.draw.lines(sc, (64, 128, 255), True, [[cam.rectPosition[0], cam.rectPosition[1]], [cam.rectPosition[2], cam.rectPosition[1]], [cam.rectPosition[2], cam.rectPosition[3]], [cam.rectPosition[0], cam.rectPosition[3]]], 2)
    pos = pygame.mouse.get_pos()
    text = font.render("Minerals: "+str(cam.mousemotion)+"Minerals: "+str(cam.rectPosition),True,black)
    sc.blit(text, [0,0]) 
    time.sleep(0.01)
    pygame.display.flip()
