        ...
        for self.object in mainers:
             ...
             if self.cheker == 2:
                  ...
                  if self.onclick == 1 and self.choosObject[-1].takeclick == 1:
                         if len(targets)>0 and self.lshift == 0: targets.clear()
                         targets.insert(0, Target((pos_m[0]-cam.pos[0],pos_m[1]-cam.pos[1])))
                         targets[0].amouts = len(self.choosObject)
                         for select_object in self.choosObject:
...
class Mainer:
    ...
    def getChosen(self):
        ...
        if self.take == 3:
    	     self.choose = targets[self.order_t]
    ...
    def getVector(self):
        ...
        if distance_C < self.choosItem.r_size: 
                    if self.take == 0:
                         ...
                         if len(targets)>self.order_t*(-1): self.order_t-=1
                         else:						 
...
while True:
    ...
    text = font.render("Minerals: "+str(cam.lshift)+"Minerals: "+str(targets),True,black)
    sc.blit(text, [0,0]) 
    time.sleep(0.01)
    pygame.display.flip()
